...
const sidebarAddCardFixedBtn = document.getElementById(
  "sidebarAddCardFixedBtn"
); // Botão "Adicionar Cartão" na sidebar
const sidebarFilterItems = document.querySelectorAll(
  ".sidebar-section.my-boards-section .sidebar-menu-item"
); // Filtros de status na sidebar
    ...
    if (list.events.length === 0) {
      ...
      list.events.forEach((event) => {
        ...
        card.innerHTML = `
                    ...
                    <div class="card-meta">
                        ${
                          event.data_evento && event.hora_inicio
                            ? `<span class="card-meta-item due-date"><i class="far fa-clock"></i> ${event.data_evento} ${event.hora_inicio}</span>`
                            : ""
                        }
                        ${
                          event.descricao
                            ? `<span class="card-meta-item" title="Descrição"><i class="far fa-file-alt"></i></span>`
                            : ""
                        }
                        ${
                          event.local
                            ? `<span class="card-meta-item" title="Local"><i class="fas fa-map-marker-alt"></i></span>`
                            : ""
                        }
                        ${
                          event.concluida
                            ? `<span class="card-meta-item"><i class="fas fa-check"></i></span>`
                            : ""
                        }
                    </div>
                `;
...
async function loadAllEventsAndRenderBoards() {
  ...
  try {
    ...
    if (currentSearchTerm) {
      ...
      allEvents = allEvents.filter(
        (event) =>
          ...
          (event.descricao &&
            event.descricao.toLowerCase().includes(lowerSearchTerm)) ||
    ...
    if (currentStatusFilter !== "all") {
      allEvents = allEvents.filter(
        (event) =>
          (event.concluida === true && currentStatusFilter === "completed") ||
          (event.concluida === false && currentStatusFilter === "pending")
      );
    }
    ...
    if (allEvents.length === 0) {
      boardMainContent.innerHTML =
        '<p class="text-center text-white-50 mt-5">Nenhum compromisso encontrado com os filtros aplicados.</p>';
      return;
    }
...
addEventForm.addEventListener("submit", async (e) => {
  ...
  if (!token) {
    showAlert(
      "Você precisa estar logado para adicionar um compromisso.",
      "danger",
      addEventAlerts
    );
    return;
...
boardMainContent.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        event.mensagem || "Erro ao carregar detalhes do compromisso."
      );
    }
...
deleteEventBtn.addEventListener("click", async () => {
  ...
  if (confirm("Tem certeza que deseja excluir este compromisso?")) {
    try {
      ...
      setTimeout(() => {
        detailModal.hide();
        currentStatusFilter = "all"; // Garante que o filtro seja "todos"
        showBoardContent(); // Exibe os quadros com todas as tarefas
      }, 1500);
    } catch (error) {
